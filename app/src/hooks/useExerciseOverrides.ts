  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select('exercise_ref_id, name_override, description_override, photo_url, video_url, notes')
        .eq('user_id', userId)
      ...
      for (const row of data ?? []) {
        result[row.exercise_ref_id as string] = {
          ...
          video_url: (row.video_url as string | null) ?? undefined,
        ...
        .upsert(
          {
            ...
            video_url: changes.video_url ?? null,
...
export function useUploadExerciseVideo(userId: string) {
  return useMutation({
    mutationFn: async ({ exerciseId, file }: { exerciseId: string; file: File }) => {
      const ext = file.name.split('.').pop() ?? 'mp4'
      const path = `${userId}/${exerciseId}/video.${ext}`
      const { error: uploadError } = await supabase.storage
        .from('exercise-photos')
